...
if($lr =~ /jump/){
	...
	%SINGLE = ($FQ => ['.extendedFrags', 'nosite.extendedFrags']);
}
...
@{$PAIRED{$FQ}} != @{$NOHINDIII{$FQ}} && die;
    ...
    &cutawk("$OUTPUT$FQ$fastq$GENOME");
    #unlink "$OUTPUT$FQ$fastq$GENOME.sam";
#    print "bowtie2-align -x ./$GENOME -1 $PAIREDINPUT{$FQ}$FQ$fastq"."1.fastq -2 $OUTPUT$PAIREDINPUT{$FQ}$FQ$fastq.fastq --local --ff --norc -k 10 --no-hd -S $OUTPUT$FQ$fastq$GENOME.sam\n";

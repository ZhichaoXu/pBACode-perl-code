...
my $fq = $cf{$lr."_reads"};
my (@lines, @rm, @t, %barcode, %barcodeBrokenHINDIII, %t, $count, $i, $file, $lowQ, $Q, $readNum, $readNum2, $readNum3, $readNum4, $readNum5, $rmL, $rmR, $t) = ();
...
#	    }
		...
		if(length($lines[1]) > 20 && &SQend($lines[3], $END3,$QUALITYG, 3)) {
		    ...
		    $lines[1] =~ tr/ATCGNatcgn/TAGCNtagcn/;
		    $lines[1] = reverse($lines[1]);
...
#		    print "$lines[3]\n";
    ...
    print "$lowQ low quality barcodes, $readNum5 reads too short and $readNum4 ones with low quality non-barcode end among $readNum total reads\n$readNum3 reads with expected two ends and cut site\n$readNum2 reads with expected two ends but no cut site\n";
}

open(O, ">$OUTPUT$MERGE"."BarcodeWithoutsite.txt") || die;

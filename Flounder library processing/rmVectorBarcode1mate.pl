...
my (@lines, @rm, @t, %barcode, %barcodeBrokenHINDIII, %BRIDGELEN, %noHindIII, %RESTRICTIONSITE, %SITE, %SQ, %t, %UPSTREAM);
my ($bridgeLen, $count, $i, $lowQ, $Q, $readNum, $readNum1, $readNum2, $readNum3, $readNum4, $rmL, $rmR, $site, $sl, $slLen, $ss, $t, $upstream) = ();
my ($BARCODELEN, $BARCODELENMIN, $BARCODELENMAX) = ($cf{"barcode_length_standard"}, $cf{"barcode_length_min"}, $cf{"barcode_length_max"});
my ($MERGE, $MINILEN, $NOHINDIII, $QUALITY,  $QUALITYTRIM, $TRIMLEN, $VAR) = ('32.notCombined_', 20, 'nosite', $cf{"SQ_threshold"}, $cf{"SQG_threshold"}, $cf{"flag_length"}, $cf{"BAC_variance"});
my $OUTPUT = 'rmVectorBarcode1mate'.$fq;
$BRIDGELEN{$fq} = [1, 1]; #1bp juxtaposed to barcodes has high mutation rate
$UPSTREAM{$fq}->[0] = $cf{$lr."_upstream1"};
$UPSTREAM{$fq}->[1] = $cf{$lr."_upstream2_rc"};
$QUALITY !~ /^\d+$/ && die "SQ threshold as a number!";
$SITE{$fq}->[0] = $cf{$lr."_cutsite"};
$SITE{$fq}->[1] = '';
my %SITEBRIDGELEN;
$SITEBRIDGELEN{$fq} = 2;
...
for($i = 0; $i < @{$SQ{$fq}}; $i++) {
	...
	if(@lines == 4){
		...
		if(length($lines[1]) > length($site) + $SITEBRIDGELEN{$fq} + $BARCODELEN) {
		    @rm = &hiseq::rmSite($lines[1], $site, $SITEBRIDGELEN{$fq}, $BARCODELEN, $BARCODELENMAX, 'l');
		} else {
		    $rmL = 0;
		    $readNum4++;
		}
...
open(O, ">$OUTPUT"."BarcodeWithoutsite.txt") || die;

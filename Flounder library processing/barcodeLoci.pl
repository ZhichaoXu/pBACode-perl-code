...
my ($INPUT, $OUTPUT, $RANGE, $RANGELOCAL, $RANGESITE, $count, $barcode, $barcodeNum, $bt, $chr, $cluster, $i, $l, $loc, $location, $mate2, $num, $read, $t, %barcode, %barcodeHindIII, %barcodeLocation, %barcodeNum, %bl, %l, %pool, %t,%tt,  @cluster, @t) = ('groupBarcode', 'barcodeLocus', 10000, 1000, 20, 0);
my %INPUT = ($FQ => ['']);
exists($INPUT{$FQ}) || die;
...
while(<I>) {
    ...
    while(s/\t([_a-z\d\.]+)(_rc)?_(\d+)LOCNUM(\d+)READNUM(\d+)//i) {
	$chr = $1;
	$2 && ($chr .= $2);
	$location = $3;
	$num = $5;
	...
	$t > 5 && warn "barcode $barcode should not spread $t loci along chromosome $chr local region\n"; #mostly 6bp separated 2 hits, but errors in repeats can result in many hits 
	...
	$chr =~ /^pcc/i || ($t > 2 && warn "barcode $barcode should not spread $t loci along chromosome $chr\n");
    }
}

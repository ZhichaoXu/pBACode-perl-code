...
sub rmN($%) {
    my ($poolFile, %tl) = @_;
    my($count, $barcodeL, $barcodeR, $br, $i, $maxL, $maxR, $num, $test) = (0);
	...
	#$test && print "$barcodeL\t$barcode[0]\t", $tl{$barcodeL}->{$barcode[0]}, "\n\n";
    }
	...
	#$test && print "max read-pair $barcode[0]\t$barcodeL\t", $tr{$barcodeL}->{$barcode[0]}, "\n\n";
    }

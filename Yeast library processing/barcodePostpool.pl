...
my $OUTPUT = 'barcodePostpool'.$fq;
my $tmpFile = $OUTPUT.'N';

open LOG, ">tb.txt";
LOG->autoflush(1); 

open(TMP, "$INPUT.txt") || die "$INPUT.txt";
while(<TMP>) {
...
print LOG "processed 1l:Nr barcode-pairs by correctifying $num reads whose right barcodes differ by no more than 2bp from other multi-read right barcodes in the 1l:Nr group\n";
...
print LOG "processed Nl:1r barcode-pairs by correctifying $num reads whose left barcodes differ by no more than 2bp from other multi-read left barcodes in the Nl:1r group\n";
...
#	$i =~ /0000$/ && print "$i right barcodes have been paired processed\n";
	...
	(exists($convert[$k]->{$bh})) && ($barcode1bpMid[$k]->{$bh} = 1);
    }
}
%tl = ();
...
BR: while(($br, $bh) = each (%mismatchR)) {
	...
	while(($barcode, $num) = each (%{$bh})) {
		...
		if(&hiseq::nomorethan1indelxS($barcode[$i], $barcode, $DSinternal)) {
		    #$t{$barcode[$i]} > $num || print "warning: ", $convert[0]->{$bl}, ":$barcode[$i] should have more reads than $br:$barcode, $t{$barcode[$i]} $num\n";
...
print LOG "$j left barcodes differ from more eriched left barcodes by <=2 bp in case of only 1 mismatch between their right barcodes. ";
...
print LOG scalar(keys %tl), " left barcodes\n";
print LOG "$j right barcodes differ from more eriched right barcodes by <=2 bp in case of only 1 mismatch between their left barcodes. ";
print LOG scalar(keys %tl), " left barcodes left after removing them\n";
%tl = &hiseq::rmHybrid($poolFile, %tl);
print LOG scalar(keys %tl), " left barcodes after 2nd removal of hybrid\n";
%tl = &hiseq::rmN($poolFile, %tl);
print LOG scalar(keys %tl), " left barcodes after removal 1:N or N:1\n";
my (@deleteL,@deleteR) = ();
while(($bl, $bh) = each (%tl)) {
...
while(($bl, $bh) = each (%tl)) {
	...
	if(exists($convert[0]->{$bl}) && exists($convert[1]->{$br})) {
	  ...
	  #  exists($ttr{$convert[1]->{$br}}) || die;
	    ...
	    if($num > 1) {
		$ttl{$convert[0]->{$bl}}/$num < 10 && die "$bl\t", $convert[0]->{$bl}, "\t", $ttl{$convert[0]->{$bl}}, "\t$num";
		$ttr{$convert[1]->{$br}}/$num < 10 && die "$br\t", $convert[1]->{$br}, "\t", $ttr{$convert[1]->{$br}}, "\t$num";
	    }

	    #print "$bl\t", $convert[0]->{$bl}, "\t", scalar(keys %{$tl{$bl}}), " delete the 1st barcode\n";
...
while(($br, $bh) = each (%tl)) {
	...
	print O "$br:$barcode[$i]\t", $tl{$br}->{$barcode[$i]}, "\n";
        $doubt{$br.':'.$barcode[$i]} = $tl{$br}->{$barcode[$i]};
    }
